...
# Otimização de hardware do PyTorch
torch.backends.cudnn.benchmark = True

# --- CONFIGURAÇÕES BÁSICAS ---
DATASET_DIR = Path("IPqM-Fall/windowed_synchronized")

CHECKPOINT_DIR = Path("checkpoints_cnn_lstm")
...
RESULTS_FILE = RESULTS_DIR / "results_cnn_lstm.json"
...
EPOCHS = 80
LEARNING_RATE = 3e-4
DROPOUT = 0.5 # Ajustado para 0.5 (Padrão do artigo para o LSTM)
NUM_CLASSES = 2
...
class SingleSensorDataset(Dataset):
    def __init__(self, X, y):
        # Transforma de (N, 180, 8) para (N, 8, 180) - Formato exigido pela Conv1d
...
# =====================================================================
# MODELO HÍBRIDO: DeepConvLSTM (Baseado em Ordóñez & Roggen, 2016)
# =====================================================================
class DeepConvLSTM(nn.Module):
    def __init__(self, num_features):
        super().__init__()
        
        # 1. Extrator Espacial (4 camadas CNN empilhadas)
        # kernel_size=5 e padding=2 mantêm o comprimento da sequência em 180
        ...
        # 2. Rastreador Temporal (2 camadas LSTM empilhadas)
        self.lstm = nn.LSTM(
            input_size=64, 
            hidden_size=128, 
            num_layers=2, 
    ...
    def forward(self, x):
        # x shape inicial: (Batch, Canais=8, Tempo=180)
        x = self.conv_block(x)
        
        # Inverte dimensões para o LSTM: (Batch, Filtros=64, Tempo=180) -> (Batch, Tempo=180, Filtros=64)
        x = x.permute(0, 2, 1) 
        
        # Processa a sequência temporal
        lstm_out, _ = self.lstm(x)
        
        # Extrai apenas o último passo de tempo da janela de 180 amostras
...
def train_single_sensor_model(sensor_name, X_train, X_test, y_train, y_test, test_subject, fold_number, save_dir):
    ...
    model = DeepConvLSTM(len(FEATURE_COLUMNS)).to(DEVICE)
...
# =====================================================================
# BLOCO PRINCIPAL (Com Média Simples / Soft Voting)
# =====================================================================
if __name__ == "__main__":
    ...
    for fold_number, (train_idx, test_idx) in enumerate(pbar, start=1):
        ...
        # Média Simples (Soft Voting) mantida conforme solicitado
    ...
    # Treinar os Modelos Finais de Produção
    print("\nTreinando Modelos Finais para Produção (100% dos dados)...")
    ...
    print("\nTreinamento Concluído! Resultados finais:")
    print(f"F1-Score (Ensemble Média Simples): {results['ENSEMBLE']['f1_mean']:.4f}")