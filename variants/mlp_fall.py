...
CHECKPOINT_DIR = Path("checkpoints_mlp")
...
RESULTS_FILE = RESULTS_DIR / "results_mlp.json"
...
INPUT_SIZE = WINDOW_SAMPLES * len(FEATURE_COLUMNS)
...
class MLP(nn.Module):
    def __init__(self, input_size):
        super().__init__()
        # Baseline MLP from paper: 1 hidden layer with 100 neurons
...
def train_single_sensor_model(sensor_name, X_train, X_test, y_train, y_test, test_subject, fold_number, save_dir):
    ...
    # FULL BATCH SETUP: batch_size = len(dataset)
...
def train_final_model(sensor_name, X, y, save_dir):
    ...
    dataset = TensorDataset(
        torch.tensor(X.reshape(X.shape[0], -1), dtype=torch.float32), 
